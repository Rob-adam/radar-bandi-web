...
const clientUiFix=`<script id="bandovera-ui-fix">(()=>{const bandiBadge=document.getElementById('updatedBadge');if(bandiBadge)bandiBadge.style.display='none';const duplicate=document.getElementById('bandoveraLegalLinks');if(duplicate)duplicate.remove();const copyright=document.querySelector('.product-footer .copyright');if(copyright)copyright.innerHTML='© <b>BANDOVERA</b> 2025–'+new Date().getFullYear()+' — Tutti i diritti sono riservati';const privacy=document.querySelector('.product-footer [data-legal="privacy"]');if(privacy)privacy.textContent='Privacy Policy';const cookie=document.querySelector('.product-footer [data-legal="policy"]');if(cookie){cookie.textContent='Cookie Policy';cookie.addEventListener('click',()=>setTimeout(()=>{const title=document.getElementById('legalTitle');const body=document.getElementById('legalBody');if(title)title.textContent='Cookie Policy';if(body)body.innerHTML='<p><b>Cookie Policy BANDOVERA</b></p><p>BANDOVERA utilizza esclusivamente dati tecnici e memorie locali necessari al funzionamento del servizio, alla gestione della licenza e al salvataggio delle preferenze dell\\'utente.</p><p>Non vengono utilizzati cookie pubblicitari o di profilazione.</p>';},0));}})();</script>`;
