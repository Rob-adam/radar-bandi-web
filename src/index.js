...
const clientGuard=`<script id="bandovera-license-guard">(()=>{try{const raw=new URLSearchParams(location.search).get('p');if(!raw)return;const c=raw.trim().replace(/\\s+/g,'');const b=(c+'='.repeat((4-c.length%4)%4)).replace(/-/g,'+').replace(/_/g,'/');const bytes=Uint8Array.from(atob(b),x=>x.charCodeAt(0));const p=JSON.parse(new TextDecoder().decode(bytes));const code=String(p.license||'').trim();if(!code)return;fetch('/api/license/'+encodeURIComponent(code),{cache:'no-store'}).then(r=>r.json()).then(s=>{if(!s||!s.configured||!s.registered)return;if(s.active===false||s.expired){document.documentElement.innerHTML='<!doctype html><html lang="it"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>BANDOVERA · Licenza non attiva</title><style>body{margin:0;background:#07111f;color:#edf5ff;font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Arial;display:grid;place-items:center;min-height:100vh;padding:24px}.box{max-width:620px;background:#0e1b2c;border:1px solid #243a55;border-radius:18px;padding:28px;text-align:center}.box h1{margin:0 0 12px}.muted{color:#9fb2c8}.code{margin-top:16px;font-weight:800;color:#50d5ff}</style></head><body><div class="box"><h1>Licenza BANDOVERA non attiva</h1><p class="muted">Il servizio è sospeso o la licenza è scaduta. Per riattivare l’accesso è necessario il rinnovo del servizio.</p><div class="code">'+code.replace(/[&<>]/g,'')+'</div></div></body></html>';}}).catch(()=>{});}catch(e){console.warn('BANDOVERA license check',e)}})();</script>`;
...
export default {
 async fetch(request,env){
  ...
  if(url.pathname.startsWith('/api/license/')){
   ...
   if(!env.LICENSES)return json({configured:false,registered:false,active:true});
   const rec=await env.LICENSES.get('license:'+code,{type:'json'});
   if(!rec)return json({configured:true,registered:false,active:true,license:code});
   const expired=isExpired(rec.expiresAt);
   return json({configured:true,registered:true,license:code,active:rec.active!==false&&!expired,expired,expiresAt:rec.expiresAt||null});
  }
  ...
  if(url.pathname==='/api/admin/sync-status'){
   return json({configured:!!env.LICENSES,adminSecret:!!env.ADMIN_API_KEY});
  }
